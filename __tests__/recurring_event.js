...
const ftitle = faker.company.bs();
//const fdate = faker.date.future()

test("Activity Creator", async () => {
  const queryParams = {
    startDate: "2022-09-11T00:00:00Z",
    invitedFriends: [],
    title: "Recurrence Test 1",
    description: "ad",
    featuredPic:
    ...
    minAge: 20,
    maxAge: 40,
    approvalForFollow: false,
    smoking: "neutral",
    drinking: "neutral",
    eventPrivacy: "public",
    allowedGender: "all",
    startTimes: ["10:30:00Z"],
    creator: {
      connect: {
        where: {
          node: {
    ...
    recurrance: {
      dates: ["2022-09-12T00:00:00Z", "2022-09-13T00:00:00Z"],
      count: 2,
    },
    hostid: 70,
  ...
  const res = await axios({
    method: "post",
    url: `http://103.72.79.12/api/create_recurring`,
    data: queryParams,
    headers: {
      action: "create",
...
test("Activity Checker", async () => {
  ...
  const date = res.data.data.activityDetails.startTimes;
